...
每个查看窗是独立进程（`win/island_viewer.py`），毒页面或 WebView2 回归最多只影响该窗，绝不连坐主岛。查看窗为无边框自绘壳（与岛同一设计语言）：顶栏拖动、双击顶栏最大化/还原、`─ □ ✕` 三键、Esc 关闭、右下握把调大小。kind 省略时按扩展名识别；链路为 `show.sh → 桥 POST /api/show → 页面轮询 → 岛壳动态开窗`，全程本机回环。
